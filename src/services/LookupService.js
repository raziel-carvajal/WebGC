...
LookupService.prototype.apply = function (msg) {
  ...
  if (this.discoveredPaths[target]) {
    this.log.info('Handshake was already initated with: ' + target + ', enqueueing message')
...
LookupService.prototype.inOfferReception = function (msg) {
  ...
  var msge
  var i
  for (i = 0; i < msg.payload.iceCandidates.length; i++) {
    msge = {
          ...
          if (lookupInfo.originator) {
            ...
            if (path && steps) {
              ...
              if (connection) {
                ...
                this.log.error('Link between ' + this.id + ' and ' + path[steps] +
