...
(function(exports){
  ...
  * @author Raziel Carvajal <raziel.carvajal-gomez@inria.fr> */
  function Coordinator(opts){
    ...
    this.factory = new GossipFactory( {loggingServer: opts.loggingServer, 
      peerId: opts.peerId});
  ...
  * Coordinator.protocols object.*/
  Coordinator.prototype.doActiveThread = function(protocol){
    ...
    connection.on('error', function(err){
      self.log.error('During the emition of a ' + protocol.class + ' message');
      self.log.error(err);
    });
  };
  
  Coordinator.prototype.sendToPlotter = function(log){
    if( this.plotterId !== 'undefined' ){
      var self = this;
      var connection = this.connect(this.plotterId, { label: 'plotter' });
      connection.on('open', function(){
        connection.send( log );
        self.log.info('Log to Plotter was sent');
      });
      connection.on('error', function(err){
        self.log.error('During a log transmition to Plotter. ' + err);
      });
    }else{
      this.log.warn('There is no plotter identifier');
    }
  };
