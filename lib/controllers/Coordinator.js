(function(exports){
  ...
  */
  function Coordinator(opts){
    ...
    this.on('doActiveThread', function(view){
      ...
      for( i = 0; i < keys.length; i++ ){
        protocol = self.protocols[ keys[i] ];
        self.pool = window.setInterval( function(){ self.doActiveThread(protocol); }, protocol.periodTime);
      }
    });
  }
  /** 
  ...
  */
  Coordinator.prototype.doActiveThread = function(protocol){
    var log = protocol.name + '_' + protocol.loop + '_' + this.id + '_' + protocol.getLog();
    console.log( '{' + log + '}' );
    protocol.loop++;
    ...
    * peers. This connection exchanges {@link Cyclon} messages. */
    var connection = this.connect(dstPeer, { label: protocol.name });
    var self = this;
    ...
    connection.on('error', function(err){
      util.error('During the emition of a ' + protocol.name + ' message');
      util.error(err);
    });
  };
  /**
