...
(function(exports){
  ...
  * @author Raziel Carvajal Gomez <raziel.carvajal-gomez@inria.fr> */  
  function GossipFactory(logOpts, id){
    this.logOpts = logOpts;
    this.id = id;
    this.log = new Logger(logOpts);
    this.log.setOutput(id, this.constructor.name);
    this.inventory = {};
    this.gossipUtil = new GossipUtil(logOpts);
    this.gossipUtil.log.setOutput(id, this.constructor.name);
  }
  /**
  ...
  * @param {Object} opts - Configuration object of a gossip protocol. */ 
  GossipFactory.prototype.checkProperties = function(opts){
    ...
    if( typeof opts.dependencies !== 'undefined' ){
      ...
      this.log.info("Object's properites hasn't dependencies");
    }
  };
  /**
  ...
  * @param {Object} opts - Configuration object of a gossip protocol. */
  GossipFactory.prototype.createProtocol = function(opts){
    try{
      ...
      this.log.error("Gossip-based protocol wasn't created\n\t" + e.message);
    }
  };
  /**
