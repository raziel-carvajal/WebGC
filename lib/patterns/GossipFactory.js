    ...
    for( var i = 0; i < keys.length; i++ ){
      if( gossipAlgos[ keys[i] ].hasOwnProperty('attributes') ){
        ...
        for( var j = 0; j < attsKeys.length; j++ ){
          var algoId = atts[ attsKeys[j] ];
          var objExt = this.inventory[ algoId ];
          if( objExt !== 'undefined' ){
            this.inventory[ keys[i] ][ attsKeys[j] ] = objExt;
            this.log.info('Object [' + keys[i] + '] was augmented with the property [' + attsKeys[j] + ']');
          }else{
