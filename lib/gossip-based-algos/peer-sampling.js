...
SamplingService.prototype._moveOldest = function(){
  if( Object.keys(this.view).length > this.H && this.H > 0 ){
    var oldests = {}, subDict = {}, oldestKey, key;
    for( var i = 0; i < this.H; i += 1 ){
      oldestKey = gossipUtil.getOldestKey(this.view);
      oldests[ this.view[oldestKey].age ] = oldestKey;
      subDict[oldestKey] = this.view[oldestKey];
      delete this.view[oldestKey];
    }
    for( key in oldests)
      this.view[ oldests[key] ] = subDict[ oldests[key] ];
  }
}
...
SamplingService.prototype._getItemsToSend = function(iD){
  ...
  this._permuteView();
  this._moveOldest();
  var ii = int( Math.floor(this.C / 2) ) - 1;
  var keys = Object.keys(this.view);
  if( keys.length >= ii ){
    var i = 0;
    do{
